...
#else
/* taken from crc32_fast.c */
static uint32_t table[0x100], wtable[0x100*sizeof(crc32_t)];
...
void crc32_init(uint32_t* table, uint32_t* wtable) {
	for(size_t i = 0; i < 0x100; ++i)
		table[i] = crc32_for_byte(i);
	for(size_t k = 0; k < sizeof(crc32_t); ++k)
		for(size_t w, i = 0; i < 0x100; ++i) {
			for(size_t j = w = 0; j < sizeof(crc32_t); ++j)
...
int main(int argc, char **argv)
{
	...
	switch(algo)
	{
		case CRC32:
			...
			crc = crc32_end(crc);
#else
				crc32_update(table, wtable, inbuf, bufnbyte, &crc);
#endif
